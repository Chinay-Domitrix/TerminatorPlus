...
public class NeuralNetwork {
	...
	private NeuralNetwork(Map<BotNode, Map<BotDataType, Double>> profile) {
		...
		this.dynamicLR = true;
		if (profile == null) Arrays.stream(BotNode.values()).forEach(n -> this.nodes.put(n, new NodeConnections()));
		else profile.forEach((nodeType, map) -> nodes.put(nodeType, new NodeConnections(map)));
