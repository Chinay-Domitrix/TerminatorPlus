...
//		bot.setSneaking(false);
	...
	private void move(Bot bot, Player player, Location loc, Location target) {
		...
		try {
			...
			if (MathUtils.isNotFinite(vel)) MathUtils.clean(vel);
		}
		if (vel.length() > 1) vel.normalize();
		if (loc.distance(target) <= 5) vel.multiply(0.3);
		else vel.multiply(0.4);
		vel.setY(0.4);
		bot.jump(vel);
	}
	...
	private Player nearestPlayer(Location loc) {
		...
		for (Player player : Bukkit.getOnlinePlayers()) {
			...
			if (result == null || loc.distance(player.getLocation()) < loc.distance(result.getLocation())) result = player;
		}
		return result;
	}
}
